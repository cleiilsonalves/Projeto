        ...
        // opcao.removeAttr('disabled');
         ...
         if (cepf != '') {
             res = $.post('get_endereco_cep.php', {'cep': cepf }, function (text, status, xml) {
                 $.getJSON('http://servicos.cdtsp.com.br/notificacao/json/endcep.json', function (data) {
                     if ('<?= $zona?>' == '' && opcao.val() == 'NT') {
                         $('#msgSel').modal('show');
                         alert('este CEP nao pode ser NT!');
                         document.cadastro.modeloNt.value = 'AR';

                         //opcao.attr('disabled','disabled');

                     }
                     alert(data[0]['ENDERECO']);

                     if ('<?= $endereco?>' == '') {
                         alert('Colocar um CEP válido!');
                         alert('<?= $endereco?> aqui e o endereço da session')
                     } else {
                         document.cadastro.endereco.value = '<?= $endereco?>';
                         document.cadastro.bairro.value = '<?= $bairro?>';
                         document.cadastro.cidade.value = '<?= $cidade?>';
                         document.cadastro.uf.value = '<?= $uf?>';
                     }
                 });
             });
         } else {
...
<div class="container">
    ...
    <div class="row col-lg-8 col-md-8 ">
        ...
        <?php } ?>
        <hr>
        <form class="form-horizontal" name= "cadastro" action="controle_cadastro.php" method="post">
            ...
            <div class="form-group">
                ...
                <div class="col-sm-4">
                    <input type="text" name="cep" class="form-control" id="cep" placeholder="CEP" required="true" onchange="vericaCep()">
                </div>
            </div>
            <div class="form-group">
